...
from app.engine.residual_miner import ResidualMiner, TargetedMissCluster
...
def test_residual_miner_extracts_and_clusters_misses(sample_orders_df, mock_incumbent_ensemble):
    ...
    miner = ResidualMiner(maturity_window_days=0, min_cluster_size=5)
...
def test_residual_miner_leakage_guard(sample_orders_df, mock_incumbent_ensemble):
    ...
    miner = ResidualMiner(maturity_window_days=0, min_cluster_size=3)
    # Add forbidden columns to input
    ...
    clusters = miner.cluster_misses(df_fn, mature_df)
...
def test_strict_full_validation_cost_weighted_acceptance_gate(sample_orders_df, mock_incumbent_ensemble):
    ...
    # Rule B (Bad): Over-flags all COD orders (catches misses but causes massive False Positive insult costs)
    ...
    # Arithmetic: Blanket COD causes 70 flags on 35 RTOs (35 TP, 35 FP * 1500 * 0.15 = 7875 FP insult cost vs 8750 TP savings)
    # Result must be gated by cost delta
...
def test_shipped_holdout_rate_routing(sample_orders_df, mock_incumbent_ensemble):
    """Verifies that shipped_holdout_rate defaults to 0.0 (no holdout) and permits random holdout when specified."""
    from app.engine.router import ThreeWayRouter

    # 1. Default router has shipped_holdout_rate = 0.0
    default_router = ThreeWayRouter()
    assert default_router.shipped_holdout_rate == 0.0

    # 2. Router with 100% holdout for testing
    holdout_router = ThreeWayRouter(shipped_holdout_rate=1.0)
    assert holdout_router.shipped_holdout_rate == 1.0

    # Active high-risk rule
    high_risk_rule = RuleHypothesis(
        id="hyp_high_risk",
        name="High Risk COD Rule",
        code="def predict(df):\n    return df['payment_mode'] == 'COD'",
    )
    ensemble = EnsembleRule([high_risk_rule, high_risk_rule])  # 2 matching rules triggers AUTO_BLOCK

    # With default router (0% holdout): COD orders are blocked
    ...
    # With 100% holdout: High-risk orders are exempted and AUTO_APPROVED for outcome observation
